...
@MainActor final class WindowStore: ObservableObject {
    ...
    func analyze() { guard let selected,let frame=selected.ax.frame else{return}; isAnalyzing=true; Task { do { let map=try await Phase2Capture.captureMap(excluding:selected.cg?.id); let ratio=frame.width/frame.height; let result=PlacementOptimizer.best(map:map,aspectRatio:ratio,minSize:CGSize(width:240,height:240/ratio),maxSize:CGSize(width:map.bounds.width*0.55,height:map.bounds.height*0.55),costBudget:0.32); proposedPlacement=result?.frame; heatmap=ScoringMapGenerator.heatmap(map,placement:result?.frame).map{NSImage(cgImage:$0,size:.zero)}; if let result { setGeometryFields(result.frame) } else { errors.insert("No placement satisfied the scoring-map cost budget.",at:0) } } catch { errors.insert("Screen analysis failed: \(error)",at:0) }; isAnalyzing=false } }
