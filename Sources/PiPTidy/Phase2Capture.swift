...
enum Phase2Capture {
    static func captureMap(excluding windowID: CGWindowID?) async throws -> PlacementMap {
        ...
        let foregroundMap = ScoringMapGenerator.applyingForegroundPriority(baseMap, windows:SystemCGInventory().enumerate(), excludingPID:getpid(), excludingWindowID:windowID)
