...
public enum PlacementOptimizer {
    ...
    public static func best(map:PlacementMap, aspectRatio:CGFloat, minSize:CGSize, maxSize:CGSize, costBudget:Double) -> PlacementResult? {
        guard map.width>0,map.height>0,aspectRatio>0 else{return nil}; var integral=[Double](repeating:0,count:(map.width+1)*(map.height+1)); for y in 0..<map.height { for x in 0..<map.width { integral[(y+1)*(map.width+1)+x+1]=Double(map.cost(atX:x,y:y))+integral[y*(map.width+1)+x+1]+integral[(y+1)*(map.width+1)+x]-integral[y*(map.width+1)+x] } }
