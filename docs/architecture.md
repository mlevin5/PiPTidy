...
Phase 2 takes a user-triggered ScreenCaptureKit screenshot, excluding PiP Tidy and the selected PiP window. A pure map generator weights edges, luminance, saturation, and centrality; menu-bar and Dock regions receive maximum cost. The UI previews the heatmap and exact rectangle before a separate user action moves the window.
