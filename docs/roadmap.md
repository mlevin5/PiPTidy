...
- [x] **Desktop-aware clear-space detection.** Use the known bounds of on-screen windows to identify heatmap regions showing only the desktop background and treat those regions as clear placement candidates, regardless of wallpaper detail. PiP and PiP Tidy's own windows are excluded from the coverage mask.
